...
class cMenu:
   ...
   def __init__(self, x, y, h_pad, v_pad, orientation, number, background,
      ...
      self.u_color = WHITE_RGB                      # Color for unselected text
      self.s_color = RED_RGB                        # Color for selected text
      self.image_highlight_color = BLUE_RGB         # Color for the image highlights
   ...
   def create_button(self, button_info):
      ...
      if button_info[2] is None:
         if button_info[3] is None:
            width, height = self.font.size(button_info[0])
         else:
            width, height = self.font.size(button_info[0] + max(button_info[3], key=len) + ' ')
            if button_info[4] is not None:
                default = button_info[4]
   ...
   def set_button_images(self):
      for button in self.menu_items:
         ...
         if button['b_image'] == None:
            ...
            if button['options']:
               text_image   = r(button['text'] + button['options'][button['opt_ind']], True, self.s_color)
            else:
            ...
            if button['options']:
               text_image   = r(button['text'] + button['options'][button['opt_ind']], True, self.u_color)
            else:
   ...
   #
   def update(self, e, c_state, p_state):
      redraw_full_menu = False
      reset_button_images = False
      ...
      if e.key == pygame.K_DOWN:
         ...
         if self.menu_items[self.selection]['options'] and o == 'vertical':
            ind = self.menu_items[self.selection]['opt_ind']
            if ind + 1 < len(self.menu_items[self.selection]['options']):
               self.menu_items[self.selection]['opt_ind'] += 1
            else:
               self.menu_items[self.selection]['opt_ind'] = 0
            reset_button_images = True
         ...
         if self.menu_items[self.selection]['options'] and o == 'vertical':
            ...
            if ind - 1 >= 0:
               ...
               self.menu_items[self.selection]['opt_ind'] = len(self.menu_items[self.selection]['options']) - 1
            reset_button_images = True
